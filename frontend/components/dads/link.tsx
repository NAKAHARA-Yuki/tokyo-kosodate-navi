...
//       src/components/Link/Link.tsx （commit 88110f7, package v2.7.0）
...
export const Link = (props: LinkProps) => {
  ...
  const anchorRest = rest as ComponentProps<"a">;

  return (
    <a className={`${linkStyle} ${className ?? ""}`} {...anchorRest}>
      {children}
      {anchorRest.target === "_blank" && <LinkExternalLinkIcon {...icon} />}
    </a>
  );
};
