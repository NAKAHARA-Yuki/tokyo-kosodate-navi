...
ポートだけは自動で分けられないので、2人目以降は `.env` で変えてください。

```bash
echo "DEV_PORT=8081" >> .env    # make dev のポート
```
...
Claude Code に作業を任せて離席するなら、毎回の承認を省けます。
`.claude/settings.local.json` を作ってください。
...
このファイルは **git 管理外**（`.gitignore` 済み）なので、各自で置く必要があります。
共有していないのは意図的です。**この設定はリポジトリを開いた場所すべてで効くため**、
コンテナの外＝ホストで起動したときも承認なしになります。
ホストでは被害範囲が閉じていないので、全員に強制すべき設定ではありません。

**コンテナの中で使う前提の設定です。** 中でなら壊しても作り直せますし、
